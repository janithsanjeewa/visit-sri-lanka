          ...
          {block.items.map((item, i) => (
            <div key={i} className="flex gap-4 bg-gray-50 rounded-xl p-5">
              <div className="flex-shrink-0 text-center">
                ...
                <span className="block font-semibold text-gray-700 text-xs mt-0.5 whitespace-nowrap">{item.location}</span>
              </div>
  ...
  if (!post) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center">
        ...
        <Link to="/blog" className="text-emerald-600 font-semibold hover:underline">← Back to Blog</Link>
      </div>
    );
  }
    ...
    <>
      ...
      <section className="relative h-[50vh] min-h-[340px] overflow-hidden">
        ...
        <div className="absolute inset-0 flex flex-col justify-end p-8 max-w-4xl mx-auto left-0 right-0">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            <span className={`inline-block text-xs font-bold px-3 py-1 rounded-full mb-3 ${categoryColors[post.category] || "bg-white/20 text-white"}`}>
              {post.category}
            </span>
      ...
      <article className="max-w-3xl mx-auto px-6 py-12">
        ...
        <div className="mt-12 bg-gradient-to-br from-emerald-600 to-teal-700 rounded-2xl p-8 text-white text-center">
          ...
          <p className="text-emerald-100 mb-6">Find and book the best hotels for your trip — compare hundreds of options and get the best price.</p>
          <a
